...
class Planner(torch.nn.Module):
    ...
    def forward(self, img):
        ...
        return (B,2)
        """
        z = (img - self.input_mean[None, :, None, None].to(img.device)) / self.input_std[None, :, None, None].to(
            img.device)
        up_activation = []
        ...
        ln_z = self.classifier(z)
        # print("ln_z shape is ", ln_z.shape)
        ln_z = ln_z.squeeze(1, 2, 3)
        # print("ln_z shape after s ", ln_z.shape)
        am_pt = spatial_argmax(ln_z)
        # print("am_pt Shape", am_pt.shape)
        return am_pt
