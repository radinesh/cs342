...
def load_data(dataset_path=DATASET_PATH, transform=dense_transforms.ToTensor(), num_workers=0, batch_size=128):
    print("Data Path is ", dataset_path)
    ...
    def rollout(self, track, controller, planner=None, max_frames=1000, verbose=False, data_callback=None):
        ...
        for t in range(max_frames):
            ...
            if verbose:
                ...
                # Start added new code that need to be removed  when submitting the code
                ax.set_xticks(np.arange(0, self.config.screen_width, self.config.screen_width / 4),
                              np.arange(-1, 1, 0.5))
                ax.set_yticks(np.arange(0, self.config.screen_height, self.config.screen_height / 4),
                              np.arange(-1, 1, 0.5))
                # end added
                ...
                # need to remove code
                ax.add_patch(plt.Rectangle((1, 1), 61, 51, fc=(0, 0, 0, 0.5)))
                ax.text(2, 5, f"velocity: {current_vel:.2f}", color='w')
                ax.text(2, 10, f"drift: {action.drift}", color='w')
                ax.text(2, 15, f"nitro: {action.nitro}", color='w')
                ax.text(2, 20, f"target x: {aim_point_image[0]:.4f}", color='w')
                ax.text(2, 25, f"target y: {aim_point_image[1]:.4f}", color='w')
                ax.text(2, 30, f"time trial: {state.time:.1f}s", color='w')
                # ax.text(2, 35, f"my_param_1: {control_data['my_param_1']:.2f}", color='w')
                # ax.text(2, 40, f"my_param_2: {control_data['my_param_2']:.2f}", color='w')
                # ax.text(2, 45, f"my_param_3: {control_data['my_param_3']:.2f}", color='w')
                # ax.text(2, 50, f"my_param_4: {control_data['my_param_4']:.2f}", color='w')
                # remove this code at the time of submit
                if planner:
...
if __name__ == '__main__':
    ...
    for track in args.track:
        ...
        while n < args.steps_per_track:
            steps, how_far = pytux.rollout(track, noisy_control, max_frames=1000, verbose=args.verbose,
